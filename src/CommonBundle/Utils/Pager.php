...
abstract class Pager implements \Iterator, \Countable
{
     ...
     */
    public function __construct(QueryBuilder $qb, $maxPerPage = 10/* ,$class=null */)
    {
        //$this->setClass($class);
    ...
    /**
     * Sets the current QueryBuilder.
     *
     * @return QueryBuilder
     */
    public function setQueryBuilder(QueryBuilder $qb)
    {
        return $this->queryBuilder = $qb;
    }
     ...
     */
    public function setNbLinks($nb_links)
    {
    ...
    {
        $links = array();
        $tmp = $this->page - floor($this->nb_links / 2);
        $check = $this->lastPage - $this->nb_links + 1;
        $limit = $check > 0 ? $check : 1;
        $begin = $tmp > 0 ? ($tmp > $limit ? $limit : $tmp) : 1;

        $i = (int) $begin;
        while ($i < $begin + $this->nb_links && $i <= $this->lastPage) {
            $links[] = $i++;
        }
